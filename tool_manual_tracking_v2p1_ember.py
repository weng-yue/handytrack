...
PIC_FOLDER = r"/Volumes/ppsd_HD_MN/proj_ember/como_08-13-25/calibration_299_10fps/cam1"
TRACK_FOLDER = r"/Volumes/ppsd_HD_MN/proj_ember/como_08-13-25/calibration_299_10fps"
...
class ManualTifTracker:
    def __init__(self, root):
        ...
        tk.Button(control_row1, text="New Track", command=self.start_new_track).pack(side="left", padx=5)
        tk.Button(control_row1, text="Save CSV", command=self.save_csv).pack(side="left", padx=5)
        self.toggle_thresh_button = tk.Button(control_row1, text="Threshold?", command=self.toggle_threshold)
        ...
        self.show_saved_button = tk.Button(control_row2, text="Show Saved Tracks", command=self.toggle_show_saved_tracks)
    ...
    def toggle_threshold(self):
        ...
        self.toggle_thresh_button.config(text="Threshold disabled" if not self.threshold_enabled else "Threshold enabled")
        self.show_frame()
    ...
    def load_tif_folder(self, folder):
        ...
        self.root.after(100, self.show_frame)  # Delayed call to allow canvas size to be initialized
    ...
    def show_frame(self):
        ...
        if img_width == 0 or img_height == 0:
            print(f"Warning: Invalid image size for {path}")
            return
    ...
    def toggle_show_saved_tracks(self):
        ...
        self.show_saved_button.config(text="Hide Saved Tracks" if self.show_saved_flag else "Show Saved Tracks")
        self.show_frame()
