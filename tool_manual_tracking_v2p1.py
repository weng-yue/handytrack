...
class ManualTifTracker:
    def __init__(self, root):
        ...
        # First control row
        control_row1 = tk.Frame(self.root)
        control_row1.pack(fill="x", side="top")
        tk.Button(control_row1, text="New Track (n)", command=self.start_new_track).pack(side="left", padx=5)
        tk.Button(control_row1, text="Save CSV (s)", command=self.save_csv).pack(side="left", padx=5)
        self.toggle_thresh_button = tk.Button(control_row1, text="Threshold (t)", command=self.toggle_threshold)
        self.toggle_thresh_button.pack(side="left", padx=5)
        tk.Label(control_row1, text="Skip Gap:").pack(side="left")
        self.gap_var = tk.IntVar(value=1)
        tk.Spinbox(control_row1, from_=1, to=100, textvariable=self.gap_var, width=5, state="readonly").pack(side="left")
        self.track_len_label = tk.Label(control_row1, text="Track Length: 0")
        self.track_len_label.pack(side="left", padx=10)

        # Second control row
        control_row2 = tk.Frame(self.root)
        control_row2.pack(fill="x", side="top")
        self.show_saved_history = tk.BooleanVar(value=False)
        tk.Checkbutton(control_row2, text="Show Track History", variable=self.show_saved_history, command=self.show_frame).pack(side="left", padx=5)
        self.show_saved_button = tk.Button(control_row2, text="Show Saved Tracks (h)", command=self.toggle_show_saved_tracks)
        self.show_saved_button.pack(side="left", padx=5)
        self.frame_label = tk.Label(control_row2, text="Frame: 0")
        self.frame_label.pack(side="left", padx=10)
        self.coord_label = tk.Label(control_row2, text="X: -, Y: -")
        self.coord_label.pack(side="right", padx=10)
        self.total_tracks_label = tk.Label(control_row2, text="Total Tracks: 0")
        ...
        # Frame jump input
        jump_frame_row = tk.Frame(self.root)
        jump_frame_row.pack(fill="x", pady=5)
        tk.Label(jump_frame_row, text="Go to Frame:").pack(side="left", padx=5)

        # Use a StringVar for the entry to control its value
        self.jump_var = tk.StringVar()
        self.jump_entry = tk.Entry(jump_frame_row, width=6, textvariable=self.jump_var, validate="key")
        self.jump_entry.pack(side="left", padx=5)

        # Only allow numeric input
        self.jump_entry['validatecommand'] = (self.jump_entry.register(self.validate_number), '%P')

        tk.Button(jump_frame_row, text="Go", command=self.jump_to_frame).pack(side="left", padx=5)
    ...
    def toggle_threshold(self):
        ...
        self.toggle_thresh_button.config(text="Threshold (t)" if self.threshold_enabled else "Threshold (t)")
        self.show_frame()
    ...
    def show_frame(self):
        ...
        for i, (x, y) in enumerate(points):
            outline = "red" if i == len(points) - 1 else "green"
        ...
        self.update_jump_entry()  # Update entry whenever frame changes
